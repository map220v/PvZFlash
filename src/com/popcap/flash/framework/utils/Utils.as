package com.popcap.flash.framework.utils
{
   ...
   public class Utils
   {
      ...
      private static function splitText(str:String, font:FontInst, bounds:Rectangle) : Array
      {
         ...
         var aNumLines:int = lines.length;
      ...
      public static function createStringImage(str:String, font:FontInst, bounds:Rectangle, justification:Number) : ImageInst
      {
         ...
         for(i = 0; i < aNumLines; i++)
         {
            ...
            aImgWidth = aImgWidth < aLineWidth?Number(aLineWidth):Number(aImgWidth);
         }
         if(aImgWidth == 0)
         {
            return null;
         ...
         aImgHeight = aImgHeight + (aNumLines - 1) * (font.getAscent() + font.getLineSpacing());
         if(aImgHeight == 0)
         {
            return null;
         }
      ...
      public static function createMergedImage(images:Array, sourceRects:Array, destPoints:Array) : ImageInst
      {
         ...
         for(var i:int = 0; i < len; i++)
         {
            ...
            t = dest.y;
            b = dest.y + src.height;
            bounds.left = l < bounds.left?Number(l):Number(bounds.left);
            bounds.right = r > bounds.right?Number(r):Number(bounds.right);
            bounds.top = t < bounds.top?Number(t):Number(bounds.top);
            bounds.bottom = b > bounds.bottom?Number(b):Number(bounds.bottom);
         }
