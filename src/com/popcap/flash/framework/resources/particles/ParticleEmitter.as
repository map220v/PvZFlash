package com.popcap.flash.framework.resources.particles
{
   ...
   public class ParticleEmitter
   {
      ...
      public static const CIRCLE_FULL:Number = 6.28318530717959;
      ...
      private function updateSystemField(field:ParticleField, time:Number, index:int) : void
      {
         ...
         var aInterpX:Number = this.mSystemFieldInterp[index][0];
         var aInterpY:Number = this.mSystemFieldInterp[index][1];
      ...
      private function deleteNonCrossFading() : void
      {
         ...
         var aNumParticles:int = this.mParticleList.length;
      ...
      {
         var value:Number = track.evaluate(this.mSystemTimeValue,this.mTrackInterp[index]);
         return value;
      }
      ...
      private function spawnParticle(index:int, spawnCount:int) : Particle
      {
         ...
         var aParticleFieldCount:int = this.mEmitterDef.mParticleFields.length;
         ...
         aParticle.mParticleDuration = 1 > aParticle.mParticleDuration?Number(1):Number(aParticle.mParticleDuration);
         ...
         if(aType == ParticleEmitterType.CIRCLE_PATH)
         {
            ...
            aLaunchAngle = aLaunchAngle + this.mEmitterDef.mLaunchAngle.evaluate(this.mSystemTimeValue,aLaunchAngleInterp);
         }
         ...
         else if(aType == ParticleEmitterType.BOX)
         {
      ...
      {
         var value:Number = track.evaluate(particle.mParticleTimeValue,particle.mParticleInterp[index]);
         return value;
      }
      
      private function clamp(value:Number, min:Number, max:Number) : Number
      {
         var tmp:Number = value < max?Number(value):Number(max);
         return tmp > min?Number(tmp):Number(min);
      }
      ...
      private function renderParticle(g:Graphics2D, particle:Particle, params:ParticleRenderParams) : void
      {
         ...
         aImage.setColor(!!params.mAlphaIsSet?Number(params.mAlpha):Number(1),!!params.mRedIsSet?Number(params.mRed):Number(1),!!params.mGreenIsSet?Number(params.mGreen):Number(1),!!params.mBlueIsSet?Number(params.mBlue):Number(1));
      ...
      public function update() : void
      {
         ...
         var aSystemFieldCount:int = this.mEmitterDef.mSystemFields.length;
      ...
      private function updateParticleField(particle:Particle, field:ParticleField, time:Number, index:int) : void
      {
         ...
         aInterpX = particle.mParticleFieldInterp[index][0];
         aInterpY = particle.mParticleFieldInterp[index][1];
         ...
         switch(field.mFieldType)
         {
            case ParticleFieldType.ACCELERATION:
               particle.mVelocity.x = particle.mVelocity.x + x * SECONDS_PER_UPDATE;
               particle.mVelocity.y = particle.mVelocity.y + y * SECONDS_PER_UPDATE;
               break;
            ...
            case ParticleFieldType.ATTRACTOR:
               ...
               particle.mVelocity.x = particle.mVelocity.x + aDiffX * SECONDS_PER_UPDATE;
               particle.mVelocity.y = particle.mVelocity.y + aDiffY * SECONDS_PER_UPDATE;
               break;
            ...
            case ParticleFieldType.VELOCITY:
               particle.mPosition.x = particle.mPosition.x + x * SECONDS_PER_UPDATE;
               particle.mPosition.y = particle.mPosition.y + y * SECONDS_PER_UPDATE;
               break;
            case ParticleFieldType.POSITION:
               ...
               particle.mPosition.x = particle.mPosition.x + (x - aLastX);
               particle.mPosition.y = particle.mPosition.y + (y - aLastY);
               break;
            case ParticleFieldType.GROUND_CONSTRAINT:
               if(particle.mPosition.y > this.mSystemCenter.y + y)
               {
                  ...
                  particle.mVelocity.x = particle.mVelocity.x * aCollisionReflect;
      ...
      public function initFromDef(emitterDef:ParticleEmitterDefinition, system:ParticleSystem) : void
      {
         ...
         this.mSystemDuration = this.mSystemDuration > 1?Number(this.mSystemDuration):Number(1);
         var aSystemFieldCount:int = this.mEmitterDef.mSystemFields.length;
         ...
         var aNumInterps:int = this.mTrackInterp.length;
      ...
      private function updateParticle(particle:Particle) : Boolean
      {
         ...
         if(particle.mParticleAge >= particle.mParticleDuration)
         {
            ...
            else if(particle.mCrossFadeDuration > 0)
            {
         ...
         var aFieldCount:int = this.mEmitterDef.mParticleFields.length;
         ...
         particle.mPosition.x = particle.mPosition.x + particle.mVelocity.x;
      ...
      private function updateSpawning() : void
      {
         ...
         anInterp = aSpawnEmitter.mTrackInterp[TRACK_SPAWN_RATE];
         this.mSpawnAccum = this.mSpawnAccum + aTrack.evaluate(aTime,anInterp) * SECONDS_PER_UPDATE;
         var aSpawnCount:int = int(this.mSpawnAccum);
         this.mSpawnAccum = this.mSpawnAccum - aSpawnCount;
         aTrack = aDef.mSpawnMinActive;
         aTime = aSpawnEmitter.mSystemTimeValue;
         anInterp = aSpawnEmitter.mTrackInterp[TRACK_SPAWN_MIN_ACTIVE];
         ...
         aTime = aSpawnEmitter.mSystemTimeValue;
         anInterp = aSpawnEmitter.mTrackInterp[TRACK_SPAWN_MAX_ACTIVE];
         ...
         if(aDef.mSpawnMaxLaunched.isSet())
         {
            ...
            aTime = aSpawnEmitter.mSystemTimeValue;
            anInterp = aSpawnEmitter.mTrackInterp[TRACK_SPAWN_MAX_LAUNCHED];
